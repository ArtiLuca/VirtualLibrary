 ...
 *  Il rating e' sempre preso in considerazione e poi vengono usati gli attributi ritenuti piu' importanti per determinare la qualita' di un media come percepita dall'utente.
 *
 ...
 *  come ad esempio 'normalizeVideoResolution' nel caso dei media Video.
 *  In molti casi la normalizzazione degli attributi utilizza valori di sogli massimi/minimi, percebibili dall'utente, utilizzati per indicare i limiti per i valori, che tipicamente,
 *  quando oltrepassati (in basso o in altro) non presentano differenze di qualita' percepibili.
 *
 *  Il calcolo per i vari tipi di media avviene secondo il seguente comportamento:
 *
 *   - Audio: rating(40%), qualita' audio (50%), format (10%)
 *     In particolare la qualita' audio utilizza il helper privato 'normalizeAudioQuality' che restituisce una media ponderata dei attributi bitrate, samplerate, bitdepth e channels.
 *   - Video: rating (35%), risoluzione video (45&), framerate (10%), supporto HDR (10%)
 *     In particolare, la risoluzione video utilizza l'helper privato 'normalizeVideoResolution'.
 *   - EBook: rating (100%)
 *   - Image: rating (30%), megapixel immagine (40%), compressione (20%), bitdepth (10%)
 *     In particolare, viene utilizzato l'helper privato 'normalizeImageQuality'.
 *
 *  Le informazioni relative allo "scoring" di un media vengono salvate nella stringa 'scoreInfo'.
 *
 *  ScoreVisitor dispone di un costruttore di default, getter dei campi dati privati, un metodo di reset e altri helper per il calcolo dello score dei specifici tipi di media.
 *  ScoreVisitor e' concreta in quanto va a rifefinire i metodi virtuali puri ereditati da IConstVisitor, in modo specifico per ogni tipo di media.
 *
 */
