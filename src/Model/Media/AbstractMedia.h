 ...
 *  Oltre agli attributi ereditati ad AbstractFile, definisce altri attributi ritenuti comuni a tutti i media, tra cui il nome, l'uploader, il format, e un campo rating (opzionale) del media.
 *
 *  AbstractMedia dispone di un costruttore di default, di , un costruttore di copia, la ridefinizione dell'operatore di assegnazione, un distruttore virtuale
 *  e vari getter/setter per i campi dati.
 *
 ...
 *
 *  I metodi virtuali puri 'fromJson' e 'toJson' vengono ridefiniti per implementare la persistenza dei dati (JSON serialization) e sono gli unici metodi del modello in cui vengono usati elementi Qt.
 *
...
public:
    ...
    /** @brief toJson : virtuale puro per la persistenza dati con JSON (scrittura) */
    virtual QJsonObject toJson() const = 0;

    /** @brief fromJson : virtuale puro per la persistenza dati con JSON (lettura) */
...
protected:
    ...
    /** @brief AbstractMedia : costruttore di default di AbstractMedia, imposta campi dati ai valori di default */
    AbstractMedia();
