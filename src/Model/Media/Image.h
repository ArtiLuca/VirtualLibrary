 ...
 *  A differenza del tipo Video, che ha un aspect ratio vincolato a '16:9', un media Image puo' avere un aspect ratio diverso.
 *
 ...
 *  l'aspect ratio, il bitdepth (espresso in bit) dell'immagine, dall'essere compressa oppure no e un campo opzionale per indicare il luogo dove e' stata creata/scattata.
 ...
 *
 *  In questa libreria virtuale, un media di tipo Image e' considerato valido se:
 *      - dispone di una risoluzione ma non di una durata
 *      - contiene tutti i campi impostati (con l'eccezione del campo 'locationTaken')
 *      - ha un aspect ratio che rispecchia la risoluzione impostata (con l'eccezione di alcuni casi particolari)
 *      - ha sia larghezza che altezza della risoluzione impostata nell'intervall [200-8000]
...
public:
    ...
    // === METODI SPECIFICI Image --- DA TOGLIERE --- ===

    /** @brief getAspectRatioAsFloat : ritorna l'aspect ratio come float */
    float getAspectAsFloat() const;

    /**
     * @brief getTotalMegapixels : restituisce il numero totale di megapixels del media Image (in base alla risoluzione
    ...
    /** @brief toJson : ridefinizione virtuale puro ereditato da AbstractMedia per la persistenza dei dati in JSON (scrittura) per il tipo Image (commentato nella definizione) */
    QJsonObject toJson() const override;

    /** @brief fromJson :  ridefinizione virtuale puro ereditato da AbstractMedia per la persistenza dei dati in JSON (lettura) per il tipo Image (commentato nella definizione) */
