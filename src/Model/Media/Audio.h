 ...
 *  Un oggetto della classe Audio rappresenta un media audio, cioe' un media avente una durata (espressa in minuti) ma non una risoluzione, in quanto non avrebbe logicamente senso.
 *
 *  Un oggetto Audio rappresenta una media audio tramite il suo artista ,il genre, l'album (se presente), l'anno d'uscita, la durata (espressa in minuti), il bitrate (espresso in kbps),
 ...
 *  La classe Audio e' una classe concreta, in quanto va a ridefinire i metodi virtuali puri ereditati da IMediaLength, AbstractMedia (e quindi anche AbstractFile) in modo specifico al tipo Audio.
 *
 *  In questa libreria virtuale, un media di tipo Audio e' considerato valido se:
...
public:
    ...
    /** @brief toJson : ridefinizione virtuale puro ereditato da AbstractMedia per la persistenza dei dati in JSON (scrittura) per il tipo Audio (commentato nella definizione) */
    QJsonObject toJson() const override;

    /** @brief :  ridefinizione virtuale puro ereditato da AbstractMedia per la persistenza dei dati in JSON (lettura)  per il tipo Audio (commentato nella definizione) */
