 ...
 *  Un oggetto della classe EBook rappresenta una e-book, cioe' un media avente una durata (espressa per numero di pagine) ma non di una risoluzione.
 *
 *  Un oggetto EBook rappresenta un e-book tramite il suo autore, casa editrice ("publisher"), anno di pubblicazione, il suo ISBN-13, la sua "durata" (espressa per numero di pagine),
 *  la sua categoria, la lingua del testo, il path della copertina (se presente) e se contiene immagini oppure no.
 *
 ...
 *
 *  In questa libreria virtuale, un media di tipo EBook e' considerato valido se:
 *       - ha una durata espressa per il suo numero di pagine e non dispone di una risoluzione
...
public:
    ...
    /** @brief toJson : ridefinizione virtuale puro ereditato da AbstractMedia per la persistenza dei dati in JSON (scrittura) per il tipo EBook (commentato nella definizione) */
    QJsonObject toJson() const override;

    /** @brief fromJson :  ridefinizione virtuale puro ereditato da AbstractMedia per la persistenza dei dati in JSON (lettura) per il tipo EBook (commentato nella definizione) */
